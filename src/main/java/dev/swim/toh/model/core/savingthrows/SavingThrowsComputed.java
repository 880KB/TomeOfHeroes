...
public class SavingThrowsComputed extends ComputedBase {
    ...
    public void addListeners() {
        for (SavingThrow savingThrow : SavingThrow.values()) {
            // Total Saving Throws
            characterModel.savingThrows.getSavingThrowBaseProperty(savingThrow).addListener((obs, oldBase, newBase) ->
                    savingThrowTotalPropertyMap.get(savingThrow).set(SavingThrowCalculator.getSavingThrowTotal(characterModel, savingThrow))
            );
            characterModel.savingThrows.getSavingThrowAttributeModProperty(savingThrow).addListener((obs, oldAttributeMod, newAttributeMod) ->
                    savingThrowTotalPropertyMap.get(savingThrow).set(SavingThrowCalculator.getSavingThrowTotal(characterModel, savingThrow))
            );
            characterModel.savingThrows.getSavingThrowMagicModProperty(savingThrow).addListener((obs, oldMagicMod, newMagicMod) ->
                    savingThrowTotalPropertyMap.get(savingThrow).set(SavingThrowCalculator.getSavingThrowTotal(characterModel, savingThrow))
            );
            characterModel.savingThrows.getSavingThrowMiscModProperty(savingThrow).addListener((obs, oldMiscMod, newMiscMod) ->
                    savingThrowTotalPropertyMap.get(savingThrow).set(SavingThrowCalculator.getSavingThrowTotal(characterModel, savingThrow))
            );
            // Attribute Mods
